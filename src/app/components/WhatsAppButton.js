...
import { generalEnquiryLink, isWhatsAppEnabled } from '@/lib/whatsapp';

/**
 * Floating WhatsApp button.
 *
 * Hidden inside /admin, where it is noise rather than help, and hidden entirely
 * when NEXT_PUBLIC_WHATSAPP_NUMBER is unset so a placeholder link never ships.
 */
export default function WhatsAppButton() {
  const pathname = usePathname();

  if (!isWhatsAppEnabled() || pathname?.startsWith('/admin')) return null;

  const href = generalEnquiryLink();
  if (!href) return null;
  ...
  return (
    <a
      ...
      className="fixed bottom-5 right-5 z-40 w-13 h-13 flex items-center justify-center rounded-full bg-[#25D366] shadow-lg shadow-black/40 hover:scale-105 active:scale-95 transition-transform"
...
/** lucide-react has no WhatsApp glyph, so the official mark is inlined. */
